...
BOOL CALLBACK inspect_child(const HWND control, const LPARAM parameter) {
    ...
    if (value == L"Version 0.18.4" || value == L"版本 0.18.4") summary.version = true;
...
int wmain(const int argument_count, wchar_t** arguments) {
    try {
        ...
        require(version_value(product, L"FileVersion") == L"0.18.4.0",
                "File version matches the project version");
        require(version_value(product, L"ProductVersion") == L"0.18.4.0",
        ...
        simpilot::AboutWindow::show_modal(
            GetModuleHandleW(nullptr), nullptr, localization, product, L"0.18.4");
        inspector.join();
