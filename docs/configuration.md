...
`F1` 至 `F24`，并明确区分左右修饰键、主键盘 Enter 和数字键盘 Enter。源主键还可
选择单独的左/右 Ctrl、Alt、Shift 或 Win；修饰键作为源主键时不能再包含源修饰键或 chord。
...
单独修饰键源使用 250 ms 消歧：出现后续按键时回放为普通组合键前缀，单独释放或
持续按住超过截止时间时才提交映射。由此可配置物理右 Ctrl 到
`左 Win + 左 Shift + F23`，同时保留快速输入的普通 `Ctrl+C` 等组合。
