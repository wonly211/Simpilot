...
上表是新增键盘映射前的 Phase 2 历史基线。2026-09-13 发布的 `v0.18.7` 正式产物为：

| 产物 | 字节数 |
|---|---:|
| 正式 `Simpilot.exe` | 1,170,432 |
| 正式 ZIP | 2,540,392 |
...
直接与 `v0.18.7` 比较。相对 `v0.18.6`，EXE 增长 `13,824` 字节（`1.1952%`），
ZIP 增长 `5,696` 字节（`0.2247%`），均未超过全局 `5%` 阈值。

同日发布的 `v0.18.8` 正式产物为：

| 产物 | 字节数 |
|---|---:|
| 正式 `Simpilot.exe` | 1,187,840 |
| 正式 ZIP | 2,547,504 |

正式 ZIP SHA-256 为
`E24652F32FCD978D9A8EF02266CD5E2F9E194F2BCD75A45BFF8B7856D4063011`。
当前基线为 `v0.18.8`，相对 `v0.18.7` 的 EXE 与 ZIP 增长分别为 `1.4873%` 和
`0.2800%`，无需 `approvedGrowth`。
