...
- 支持按物理键区分左右修饰键的单键、快捷键和单级 chord 映射；左右 Ctrl、Alt、Shift、Win 也可单独作为源键。添加或编辑时既可直接录制，也可从下拉框选择包括 F1-F24 在内的按键，并可设置全局或应用进程范围。例如可将普通键盘的右 Ctrl 映射为 `左 Win + 左 Shift + F23`，模拟 Copilot 键序列。
